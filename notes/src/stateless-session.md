...
許容される理由は、対象システムの脅威モデルでは「Cookieが盗まれたら期限まで悪用されうる」リスクの期待損失が、ストア運用のコストを下回るから（EC閲覧・コンテンツサイト・一般SaaS等）。サーバレス／エッジ実行環境（Lambda@Edge、Cloudflare Workers）では低レイテンシな共有ストアが**そもそも置けない**ため、署名検証だけで各エッジが独立に認証できる価値が支配的になる。
