...
問い「その世界に何が存在するか」を**形式化する知識工学側**のノート。[[ontology-taxonomy-choreography|KlynのIAモデル]]の第一要素の厳密版であり、UI側では[[ooui]]のオブジェクト抽出、エージェント側では[[ai-agent-moc]]の領域（ドメイン知識をLLMに渡す）と接する。
